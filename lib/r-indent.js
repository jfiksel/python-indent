...
export default class RIndent {
  // Comment for push
    properlyIndent() {
        ...
        let indentforassignment = this.endsWithAssignment(lines[row - 1]);
        if ( indentforassignment){
          let indentLevel = this.editor.indentationForBufferRow(row - 1) + 1;
        ...
        if (!(openBracketStack.length || (lastClosedRow.length && openBracketStack))) {
          let currentline = lines[row - 1];
          let previousline = "";
          let previouslinenumber = row - 2;
          if (previouslinenumber > -1) {
             previousline = lines[previouslinenumber];
          }

          let indentforpipe = this.indentPipe(currentline, previousline);
          //let indentforpipe = true;
          if ( indentforpipe ){
            let indentLevel = this.editor.indentationForBufferRow(row - 1) + 1;
            this.editor.setIndentationForBufferRow(row, indentLevel);
            return;
        ...
        if (!openBracketStack.length) {
            ...
            if (lastClosedRow[1] === row - 1) {
                ...
                let currentline = lines[lastClosedRow[1]];
                let previousclosedline = "";
                let previousclosedlinenumber = lastClosedRow[0] - 1;
                ...
                indentforplus = this.indentPlus(currentline, previousclosedline);

                deindentforplus = this.deindentPlus(currentline, previousclosedline);

                indentforpipe = this.indentPipe(currentline, previousclosedline);

                deindentforpipe = this.deindentPipe(currentline, previousclosedline);
    ...
    indentPlus(currentfunctionline, previousfunctionline){
      let indentforplus = false;
      plusatline = this.containsPlus(currentfunctionline);
      if (previousfunctionline.length > 1){
        plusaboveline = this.containsPlus(previousfunctionline);
        assignmentaboveline = this.endsWithAssignment(previousfunctionline);
    ...
    deindentPlus(currentfunctionline, previousfunctionline){
      let deindentforplus = false;
      //const c = currentfunctionline[currentfunctionline.length - 1];
      plusatline = this.containsPlus(currentfunctionline);
      if (previousfunctionline.length > 1){
        plusaboveline = this.containsPlus(previousfunctionline);
    ...
    indentPipe(currentfunctionline, previousfunctionline){
      ...
      pipeatline = this.containsPipe(currentfunctionline);
      if (previousfunctionline.length > 1){
        pipeaboveline = this.containsPipe(previousfunctionline);
        assignmentaboveline = this.endsWithAssignment(previousfunctionline);
