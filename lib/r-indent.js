...
export default class RIndent {
    properlyIndent() {
        ...
        const indentforbinary = this.indentBinary(lines);
        const deindentforbinary = this.deindentBinary(lines);
    ...
    containsPipe(line){
      let hasPipe = false;
      let chars = line.substring(line.length - 3);
      if (chars === "%>%"){
        hasPipe = true;
      }
      return hasPipe;
    ...
    containsBinary(line){
      let hasBinary = false;
      if (this.containsPipe(line) || this.containsPlus(line)){
        hasBinary = true;
      }
      return hasBinary;
    }
    ...
    indentBinary(lines){
      let indentforbinary = false;
      currentline = lines[(lines.length - 1)];
      plusatline = this.containsBinary(currentline);
      if (lines.length > 1){
        lineabove = lines[(lines.length - 2)];
        plusaboveline = this.containsBinary(lineabove);
        if(plusatline && !plusaboveline){
          indentforbinary = true;
        }
      } else {
        if (plusatline){
          indentforbinary = true;
      ...
      return indentforbinary;
    }

    deindentBinary(lines){
      let deindentforbinary = false;
      currentline = lines[(lines.length - 1)];
      plusatline = this.containsBinary(currentline);
      if (lines.length > 1){
        lineabove = lines[(lines.length - 2)];
        plusaboveline = this.containsBinary(lineabove);
        if(!plusatline && plusaboveline){
          deindentforbinary = true;
        }
      }
      return deindentforbinary;
    }
